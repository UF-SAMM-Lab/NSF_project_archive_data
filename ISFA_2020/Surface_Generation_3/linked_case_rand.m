function [jointTrajectoryR, TrajectoryR, jointTrajectoryL, TrajectoryL, timespan] = linked_case_rand()
...
J1R = [-pi*rand       pi*rand];   
J2R = [-pi/6*rand     pi/2*rand];   
J3R = [-pi/12*rand    pi/2*rand];  
J4R = [-pi/2*rand     pi/4*rand];  
J5R = [pi/2.5*rand   -pi/2*rand];   

%Range of motion for left arm
J1L = J1R; %Shared joint, must be the same
J2L = [-pi/2*rand     pi/6*rand];   
J3L = [-pi/12*rand    pi/2*rand];  
J4L = [-pi/2*rand     pi/4*rand];  
J5L = [pi/2.5*rand   -pi/2*rand]; 
